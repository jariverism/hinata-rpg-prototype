(function(global){
  'use strict';
  var RELICS={
    kat:{name:'負けず嫌いのノート',icon:'📓',effect:'育成中の成長を後押しする'},
    kum:{name:'キャプテンのメモ帳',icon:'🗒️',effect:'開始時の運営評価＋10'},
    kyo:{name:'歌姫のマイク',icon:'🎤',effect:'ボイトレ成功時の上昇量＋2'},
    mir:{name:'太陽のペンライト',icon:'☀️',effect:'ライブの思い出を照らす'},
    ...
    tak:{name:'分析ノート',icon:'📘',effect:'選抜審査点＋8'},
    ig:{name:'バラエティ笛',icon:'📣',effect:'バラエティ成功時に人気＋3'},
    mem:{name:'約束のリボン',icon:'🎀',effect:'特別な思い出の証'},
  ...
  function normalizeMeta(raw,ending){
    ...
    return {runs:Number(raw.runs)||0,owned:unique(owned),clears:raw.clears&&typeof raw.clears==='object'?raw.clears:{},centers:Number(raw.centers)||0,worlds:Array.isArray(raw.worlds)?raw.worlds:[],equipped:unique(equipped),album:Array.isArray(raw.album)?unique(raw.album):[]};
  }
