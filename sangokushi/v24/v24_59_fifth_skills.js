...
Object.assign(SKILLS,{
 ...
 '大野愛実':{name:'五期の旗印',desc:'味方全軍の士気を12上げ、2ターン攻撃力を15%強化。自身は次の敵軍フェイズの被害を20%軽減。',kind:'all'},
 ...
 '佐藤優羽':{name:'文武双全',desc:'隣接敵へ武力と知力の双方を参照する一撃。敵の攻撃力を20%低下。',kind:'target',range:1},
 '下田衣珠季':{name:'猪突猛進',desc:'隣接敵へ約2倍の猛攻。自軍も反動損害を受け、士気が10下がる。',kind:'target',range:1},
 '高井俐香':{name:'慧眼の陣',desc:'味方全軍が次の敵軍フェイズに受ける損害を25%軽減。',kind:'all'},
...
function execute(p,target=null,ally=null){
 ...
 switch(p.name){
  ...
  case '大野愛実':{alive('player').forEach(u=>{u.morale=Math.min(100,(Number(u.morale)||60)+12);u.v2459AttackBuff=Math.max(Number(u.v2459AttackBuff)||0,2)});p.v2459Guard=Math.max(Number(p.v2459Guard)||0,1);text='五期の旗印！ 全軍の士気と攻勢が高まった。';break;}
  ...
  case '高井俐香':{alive('player').forEach(u=>u.v2459Guard=Math.max(Number(u.v2459Guard)||0,1));text='慧眼の陣！ 全軍が防御態勢を整えた。';break;}
