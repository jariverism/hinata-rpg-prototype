// v24.5 — recruit enemy officers; loyalty is the base and recruiter charisma modifies it
(()=>{
 const HN=new Set((window.HINATA_WORLD||[]).map(x=>x[0]));
 const recruitChance=(target,actor)=>{
   if(target.force==='在野')return Math.max(5,Math.min(95,25+actor.cha*.55));
   const base=100-(Number(target.loy)||0);
   const charm=(Number(actor.cha)||0)-70;
   return Math.max(1,Math.min(95,base+charm/2));
 };
 window.hire=function(o){
   ...
   const ordered=ts.slice().sort((a,b)=>{
     ...
     return ae-be||recruitChance(b,o)-recruitChance(a,o)||a.name.localeCompare(b.name,'ja');
   });
   showModal(`<h2>登用</h2><p>敵国武将の成功率は、<b>100－忠誠度＋（実行武将の魅力－70）÷2</b>です。最終値は1～95％に収まります。</p><div class="choice-list">${ordered.map(t=>{const chance=recruitChance(t,o);return `<button data-v244-target="${t.name}">${typeof v241FaceHtml==='function'?v241FaceHtml(t.name):''}<span><b>${t.name}</b><br>${t.force}・${t.city}　忠誠${t.loy??'―'}　成功率${Math.round(chance)}%</span></button>`}).join('')}</div><button data-close>閉じる</button>`);
   ...
   modalCard.querySelectorAll('[data-v244-target]').forEach(b=>b.onclick=()=>{
     ...
     const chance=recruitChance(t,o);
     const success=Math.random()*100<chance;
     const ad=typeof v241Advice==='function'?v241Advice(success,`${t.name}の登用`):null;
     showModal(`<h2>${t.name}を登用</h2>${ad&&typeof v241AdviceHtml==='function'?v241AdviceHtml(ad):''}<p>所属：<b>${t.force}</b>　忠誠：<b>${t.loy??'―'}</b></p><p>成功率：<b>${Math.round(chance)}%</b>${enemy?`（基礎${Math.max(0,100-(Number(t.loy)||0))}%／魅力補正${((Number(o.cha)||0)-70)/2>=0?'+':''}${Math.round(((Number(o.cha)||0)-70)/2)}%）`:'（在野武将）'}</p><p>実行武将：<b>${o.name}</b>　魅力${o.cha}　必要金180</p><button id="v244-go" class="primary">登用を実行</button><button data-close>中止</button>`);
     if(typeof v241ApplyFace==='function')modalCard.querySelectorAll('[data-v241-face]').forEach(el=>v241ApplyFace(el,el.dataset.v241Face));
