...
// Search/recruit lifecycle: rendering/reapplying static fifth-gen data must never reset dynamic campaign state.
const sakai=ctx.state.officers.find(o=>o.name==='坂井新奈'),ono=ctx.state.officers.find(o=>o.name==='大野愛実');
Object.assign(sakai,{force:'在野',city:'上党',status:'在野',loy:41,acted:5});
Object.assign(ono,{force:'日向軍',city:'洛陽',status:'一般',loy:72,acted:5});
ctx.V2458.apply();
assert.equal(sakai.force,'在野','捜索後の坂井が在野でなくなる');assert.equal(sakai.city,'上党','捜索後の坂井が発見都市から初期都市へ戻る');assert.equal(sakai.status,'在野','捜索後の坂井の身分が変わる');assert.equal(sakai.acted,5,'坂井の行動状態が巻き戻る');
assert.equal(ono.force,'日向軍','登用した大野が在野へ戻る');assert.equal(ono.city,'洛陽','登用した大野の所属都市が巻き戻る');assert.equal(ono.status,'一般','登用した大野の身分が巻き戻る');assert.equal(ono.loy,72,'登用した大野の忠誠が初期値へ戻る');assert(ctx.V2458.lifecycleSelfTest(),'五期生の捜索・登用状態保持セルフテスト失敗');
