...
const src=fs.readFileSync('sangokushi/rogue/rogue_endless_v15.js','utf8');
...
vm.runInThisContext(src,{filename:'rogue_endless_v15.js'});
const api=window.HINATA_ROGUE_ENDLESS_V15_API;assert(api,'v15 API missing');
...
assert(!src.includes('new MutationObserver'),'start screen must not use self-triggering mutation observer');
assert(src.includes('overflow-y:auto!important'),'mobile vertical scroll guard missing');
assert(src.includes('padding-bottom:calc(240px + env(safe-area-inset-bottom))'),'mobile bottom scroll room missing');
assert(src.includes('touch-action:pan-y!important'),'mobile touch scrolling guard missing');
console.log('ROGUE v0.15 endless/mobile-start regression: PASS');
