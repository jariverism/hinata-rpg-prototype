...
var EXAMS={26:{name:'第一次選抜審査',need:43},52:{name:'第二次選抜審査',need:55},78:{name:'第三次選抜審査',need:67},104:{name:'最終選抜審査',need:79}};
...
function normalizeEnding(x){
 ...
 if(!R[id]){if(x.item==='記念の遺物'||x.item==='世界線の記念写真')id='memorial';else id=null;}
...
function normalizeRun(run){
 ...
 var exams=Array.isArray(run.exams)?run.exams.filter(function(w){return EXAMS[Number(w)];}).map(Number):[];
 return {memberId:memberIdFromRun(run),memberName:run.memberName||(run.m&&run.m.name)||(Array.isArray(run.m)?run.m[0]:'主人公'),week:Math.max(1,Number(run.week)||1),stats:st.slice(0,6).map(function(v){return clamp(Number(v)||0);}),energy:clamp(Number(run.energy)||0),staff:clamp(Number(run.staff)||0),judges:Math.max(0,Math.min(4,Number(run.judges)||0)),exams:exams,logs:Array.isArray(run.logs)?run.logs:[]};
}
...
function start(){var m=MEMBERS[selected],stats=m.stats.slice();if(meta.equipped.indexOf('memorial')>=0)stats=stats.map(function(v){return clamp(v+2);});game={memberId:m.id,memberName:m.name,week:1,stats:stats,energy:100,staff:18+(meta.equipped.indexOf('kum')>=0?10:0),judges:0,exams:[],logs:[]};ending=null;screen='play';persist();render();}
function examScore(){var avg=game.stats.reduce(function(a,b){return a+b;},0)/6;var value=avg*.65+game.staff*.25+game.stats[5]*.1+rnd(-5,5);if(meta.equipped.indexOf('tak')>=0)value+=8;return Math.round(value);}
function runExam(week){var exam=EXAMS[week];if(!exam||game.exams.indexOf(week)>=0)return null;var value=examScore(),passed=value>=exam.need;game.exams.push(week);if(passed){game.judges=Math.min(4,game.judges+1);game.staff=clamp(game.staff+4);log('【'+exam.name+'】合格！ 審査点 '+value+'（基準 '+exam.need+'）');}else{game.stats[5]=clamp(game.stats[5]-5);log('【'+exam.name+'】不合格… 審査点 '+value+'（基準 '+exam.need+'）');}return passed;}
function act(index){
 if(!game||ending)return;var a=ACTIONS[index];if(!a)return;
 if(a[1]<0){game.energy=clamp(game.energy+30);log('休養して体力を回復');}
 else{game.energy=clamp(game.energy-9);var ok=rnd(1,100)<=76;if(!ok&&meta.equipped.indexOf('ush')>=0&&rnd(1,100)<=25)ok=true;if(ok){var up=rnd(2,4);if(a[1]===1&&meta.equipped.indexOf('kyo')>=0)up+=2;if(a[1]===2&&meta.equipped.indexOf('mei')>=0)up+=2;if(a[1]===4&&meta.equipped.indexOf('aya')>=0)up+=2;game.stats[a[1]]=clamp(game.stats[a[1]]+up);game.staff=clamp(game.staff+1);if(a[1]===3&&meta.equipped.indexOf('ig')>=0)game.stats[0]=clamp(game.stats[0]+3);log(a[0]+'成功 '+NAMES[a[1]]+'＋'+up);}else{game.stats[5]=clamp(game.stats[5]-3);log(a[0]+'失敗');}}
 runExam(game.week);
 if(game.week>=104)finish();else{game.week++;persist();render();}
}
function finish(){if(!game||ending)return;runExam(104);var sc=score(),rank=sc>=90?'S':sc>=78?'A':sc>=66?'B':sc>=54?'C':'D';var titles={S:'デビュー曲センター',A:'フロントメンバー',B:'二列目選抜',C:'三列目選抜',D:'選抜入りならず'};var id=memberIdFromRun(game);if(!R[id])id='memorial';if(meta.owned.indexOf(id)<0)meta.owned.push(id);meta.runs++;if(rank==='S')meta.centers++;meta.clears[id]=rank;ending={rank:rank,title:titles[rank],score:sc,relicId:id,relicName:R[id].name,judges:game.judges};screen='play';persist();render();}
...
function nextExamText(){var weeks=[26,52,78,104];for(var i=0;i<weeks.length;i++)if(game.exams.indexOf(weeks[i])<0)return '次の選抜審査：第'+weeks[i]+'週（基準 '+EXAMS[weeks[i]].need+'）';return '全選抜審査終了';}
...
function play(){if(ending)return finalView();if(!game)return '<div class="card"><p>育成データがありません。</p><button class="btn" data-action="go" data-screen="home">出発へ戻る</button></div>';var buttons=ACTIONS.map(function(a,i){return '<button class="act" data-action="act" data-index="'+i+'"><b>'+a[0]+'</b><div class="small">'+(a[1]<0?'体力＋30':'能力上昇・体力－9')+'</div></button>';}).join('');if(game.week>=104)buttons+='<button class="act wide" data-action="finish"><b>この能力で最終センター発表へ</b></button>';return '<div class="card"><h2>'+game.memberName+'　育成中</h2><div class="small">第'+game.week+'週／104週</div>'+(EXAMS[game.week]&&game.exams.indexOf(game.week)<0?'<div class="notice"><b>'+EXAMS[game.week].name+'</b><br>今週の行動後に選抜判定が行われる。</div>':'')+statRows()+'<p><b>センター評価 '+score()+'</b></p><p class="small">選抜審査突破 '+game.judges+'/4<br>'+nextExamText()+'</p></div><div class="card"><div class="acts">'+buttons+'</div></div><div class="card"><h3>活動記録</h3><div class="log">'+(game.logs.length?game.logs.map(function(x){return '<div>第'+x.w+'週 '+x.t+'</div>';}).join(''):'<div>新しい世界線が始まった。</div>')+'</div></div>';}
